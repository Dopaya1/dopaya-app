...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const renderImageNode = useCallback((image: ImageData, index: number) => {
    ...
    return (
      ...
      >
        <div className="relative w-full h-full rounded-full overflow-hidden shadow-lg border-2 border-white/20 group-hover:border-orange-300 group-hover:shadow-xl transition-all duration-300">
          <img
            src={image.src}
            alt={image.alt}
          ...
          {isHovered && (
            <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <div className="text-center text-white px-2">
                <h4 className="text-xs font-semibold mb-1 truncate">{image.title}</h4>
                <div className="w-1 h-1 bg-orange-400 rounded-full mx-auto"></div>
              </div>
            </div>
          )}
          {/* Pulse animation for active items */}
          <div className="absolute inset-0 rounded-full border-2 border-orange-400 opacity-0 group-hover:opacity-100 group-hover:animate-ping transition-opacity duration-300"></div>
        </div>
      </div>
    );
      ...
      >
        ...
        >
          <div className="relative aspect-square">
            ...
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-3 right-3 w-10 h-10 bg-white/90 backdrop-blur-sm rounded-full text-gray-700 flex items-center justify-center hover:bg-white hover:scale-110 transition-all cursor-pointer shadow-lg"
            >
              <X size={18} />
            </button>
  ...
  return (
    <>
      ...
      >
        {/* Background gradient */}
        <div className="absolute inset-0 rounded-full bg-gradient-to-br from-orange-50 via-white to-blue-50 opacity-30"></div>
        
        {/* Center dot */}
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-2 h-2 bg-orange-400 rounded-full z-20"></div>
        
        {/* Orbital rings */}
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-32 h-32 border border-orange-200 rounded-full opacity-20"></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-48 h-48 border border-orange-100 rounded-full opacity-10"></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-64 h-64 border border-orange-50 rounded-full opacity-5"></div>
        
        <div className="relative w-full h-full" style={{ zIndex: 10 }}>
          {images.map((image, index) => renderImageNode(image, index))}
        </div>
        
        {/* Loading indicator for auto-rotation */}
        {autoRotate && (
